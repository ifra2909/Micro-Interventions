...
function ScoreBars({ scores, max = 4 }: { scores: Record<MechanismId, number>; max?: number }) {
  ...
  if (rows.length === 0) return <p className="font-mono text-[11px] text-ink-muted">no dominant pattern — routing to clarity</p>;
  return (
    <div className="space-y-1.5 mt-2">
      {rows.map((m, i) => (
        <div key={m} className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] text-ink-muted w-4 text-right">{scores[m]}</span>
        </div>
      ))}
    </div>
  );
...
function ReadoutLine({ k, v, vColor = "#1a1a1a" }: { k: string; v: string; vColor?: string }) {
  return (
    <p className="font-mono text-[11.5px] leading-relaxed">
      <span className="text-ink-muted">{k}</span> <span style={{ color: vColor }}>{v}</span>
    </p>
  );
}
    ...
    <div className="panel p-7 flex flex-col items-center gap-5">
      ...
      <div className="inset-screen w-full p-5 min-h-[180px]">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`w-1.5 h-1.5 rounded-full ${stage === "analyzing" ? "bg-amber" : "bg-sage"} ${stage !== "done" ? "animate-pulse" : ""}`} />
        </div>

        {stage === "input" && !live && (
          <div className="font-mono text-[11.5px] leading-[2] text-ink-light">
            <p className="text-ink-muted">▸ system idle — awaiting input</p>
            <p className="text-ink-muted">three signals feed the match:</p>
            ...
            <p><span className="text-amber">03</span> what you've already tried</p>
            <p className="text-ink-muted">sequence: safety → clarity → growth</p>
        ...
        {live && preview && (
          <div>
            <ReadoutLine k="situation:" v={preview.situation} vColor="#7a9e7e" />
            {preview.emotions.length > 0 ? (
              ...
              <ReadoutLine k="emotion:" v="scanning…" vColor="#8a8a8a" />
            )}
            {preview.triedUnhealthy.length > 0 && (
              <ReadoutLine k="coping:" v={`↻ ${preview.triedUnhealthy[0]} — we replace, not preach`} vColor="#e8a87c" />
            )}
            {preview.triedHealthy.length > 0 && (
              <ReadoutLine k="coping:" v="✓ you've already tried something — building on it" vColor="#7a9e7e" />
            )}
            <ReadoutLine k="leading:" v={MECHANISMS[preview.mechanism].label} vColor={MECHANISMS[preview.mechanism].color} />
            {preview.gateNote && <ReadoutLine k="rule:" v={preview.gateNote} vColor="#d4b896" />}
        ...
        {stage === "analyzing" && (
          <div className="font-mono text-[11.5px] leading-[2] text-ink-light">
            {ANALYSIS_LINES.slice(0, analysisStage + 1).map((l, i) => (
              <p key={l} className={i === analysisStage ? "text-ink" : "text-ink-muted"}>
        ...
        {stage === "session" && inference && intervention && (
          <div className="pop-in">
            <ReadoutLine k="situation:" v={inference.situation} vColor="#7a9e7e" />
            ...
            {inference.triedUnhealthy.length > 0 && (
              <ReadoutLine k="coping:" v={`replacing: ${inference.triedUnhealthy.slice(0, 2).join(", ")}`} vColor="#e8a87c" />
            )}
            <ReadoutLine k="mechanism:" v={MECHANISMS[inference.mechanism].label} vColor={MECHANISMS[inference.mechanism].color} />
            <ReadoutLine k="delivered:" v={`${intervention.title} · ${intervention.minutes} min`} vColor="#1a1a1a" />
        ...
        {stage === "done" && inference && (
          <div className="pop-in font-mono text-[11.5px] leading-[2]">
            ...
            <ReadoutLine k="mechanism:" v={MECHANISMS[inference.mechanism].label} vColor={MECHANISMS[inference.mechanism].color} />
            {delta !== null && (
              <ReadoutLine k="heaviness:" v={`${delta <= 0 ? `${delta}` : `+${delta}`} — ${delta <= 0 ? "lighter. that's the whole game." : "rough round. it still counts."}`} vColor={delta <= 0 ? "#7a9e7e" : "#d4b896"} />
            )}
            <p className="text-ink-muted">the tool did its job: you felt enough to act.</p>
          </div>
        )}
      </div>
      <p className="text-center text-[11px] font-mono text-ink-muted -mt-1">
    ...
    <div className={`panel p-6 transition-opacity ${confirmed ? "opacity-70" : ""}`}>
      <div className="flex items-baseline justify-between gap-4 mb-3">
        ...
        <p className="font-display text-3xl text-ink tabular-nums">
          {value}<span className="text-ink-muted text-base font-body font-normal"> /10</span>
        </p>
      </div>
        ...
        <div className="lg:col-span-7">
            ...
            <div className="pop-in">
              <p className="mono-label flex items-center gap-2">
                <span className="relative flex w-2 h-2">
                  <span className="pulse-ring absolute inline-flex w-full h-full rounded-full bg-sage" />
                  <span className="relative inline-flex w-2 h-2 rounded-full bg-sage" />
                </span>
                evidence-based · no chatbot · under 5 minutes
              </p>
              <h1 className="font-display text-ink mt-5 leading-[1.05] text-[clamp(2.5rem,6vw,4.2rem)]">
                what's weighing<br />on you <em className="italic text-ink-light">right now?</em>
              </h1>
              <p className="text-ink-light text-[15px] sm:text-base leading-relaxed mt-5 max-w-xl">
              ...
              <div className="panel p-2 mt-10 focus-within:border-ink/20 transition-colors">
                <textarea
                  ref={taRef}
                  value={text}
                  ...
                  className="w-full bg-transparent px-4 py-3 text-ink text-[15px] leading-relaxed resize-none outline-none placeholder:text-ink-muted min-h-[110px]"
                />
                <div className="flex items-center justify-between px-3 pb-2">
                  <span className="font-mono text-[11px] text-ink-muted">{text.length}/600 · ⌘↵ to decode</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {QUICK_CHIPS.map((c) => (
                    <button
                      ...
                      className="btn-ghost px-4 py-2 text-sm hover:text-ink transition-colors"
                    >
                      {c.label}
                    </button>
                  ))}
              ...
              <p className="font-mono text-[11px] text-ink-muted mt-8 leading-relaxed">
                if this is a crisis and not a bad day — <span className="text-amber">988</span> (US) or{" "}
                <a href="https://findahelpline.com" target="_blank" rel="noreferrer" className="underline hover:text-amber transition-colors">findahelpline.com</a>.
            ...
            <div className="pop-in" key={intervention.id}>
              <div className="flex items-center justify-between gap-4">
                <p className="mono-label truncate">
                  you said: <span className="text-ink-light normal-case tracking-normal">"{manual ? "picked from library" : text.trim().slice(0, 72)}{!manual && text.trim().length > 72 ? "…" : ""}"</span>
                </p>
              ...
              <div className="mt-7">
                <h2 className="font-display text-ink leading-tight text-3xl sm:text-4xl tracking-tight">{inference.headline}</h2>
                <div className="flex items-center gap-3 mt-3">
                  <MechTag id={inference.mechanism} />
                  <span className="font-mono text-[11px] text-ink-muted">{MECHANISMS[inference.mechanism].need}</span>
                </div>
              </div>

              {inference.gateNote && (
                <p className="font-mono text-[11px] text-amber mt-3">▸ {inference.gateNote}</p>
              )}
              <p className="text-ink-light text-[15px] leading-relaxed mt-4 max-w-xl">{inference.message}</p>
              ...
              <div className="panel mt-6 overflow-hidden">
                <div className="px-7 pt-7 pb-6 border-b border-border flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <h3 className="font-display text-ink text-2xl tracking-tight">{intervention.title}</h3>
                    <p className="font-mono text-[11px] text-ink-muted mt-1.5 leading-relaxed max-w-md">
                      evidence: {intervention.evidence}
                    </p>
                  </div>
                  <span className="font-mono text-[11px] text-ink-light border border-border rounded-full px-3 py-1.5 shrink-0">
                ...
                <div className="px-7 py-6">
                  ...
                  <p className="text-ink-light text-sm leading-relaxed max-w-xl">{intervention.why}</p>
                </div>
                        ...
                        <li key={i}>
                          <button
                            ...
                            className={`w-full text-left flex items-start gap-3 px-4 py-3 rounded-xl border transition-all duration-200 ${
                              done ? "border-border bg-cream-dark" : "border-border bg-white hover:-translate-y-0.5"
                            }`}
                          >
                            <span
                              className={`w-6 h-6 rounded-full border flex items-center justify-center shrink-0 mt-0.5 transition-colors ${
                                done ? "bg-sage border-sage text-white" : "border-border text-ink-muted"
                              }`}
                            >
                              {done ? (
                            ...
                            <span className={`text-sm leading-relaxed transition-colors ${done ? "text-ink-muted line-through decoration-sage/30" : "text-ink"}`}>
                              {s}
                            </span>
                          </button>
                        </li>
                ...
                <div className="px-7 pb-7">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="font-mono text-[11px] text-ink-muted">{Math.round(progress * 100)}%</span>
                  </div>
              ...
              <div className="flex items-center justify-between gap-4 mt-5 flex-wrap">
                <p className="font-mono text-[11px] text-ink-muted italic max-w-sm">"{intervention.closer}"</p>
          ...
          {stage === "done" && inference && intervention && (
            <div className="pop-in">
              ...
              <h2 className="font-display text-ink mt-3 leading-[1.05] text-[clamp(2.2rem,5vw,3.5rem)]">
                unclenched<span className="text-sage">.</span>
              </h2>
              <p className="text-ink-light mt-4 max-w-xl text-[15px] leading-relaxed">
                You didn't solve the situation — you restored the mechanism underneath it. That's the whole
                trick: feel better <em className="text-ink not-italic font-medium">first</em>, then handle it. Because you can.
              </p>

              <div className="panel mt-10 p-7 flex flex-col sm:flex-row items-center gap-8">
                <div className="flex items-end gap-6">
                  <DeltaBar label="before" value={moodBefore ?? moodBeforeVal} color="#e8a87c" />
                  <DeltaBar label="after" value={moodAfterVal} color="#7a9e7e" />
                </div>
                <div className="flex-1 text-center sm:text-left">
                  <p className="font-display text-5xl tabular-nums" style={{ color: doneDelta !== null && doneDelta <= 0 ? "#7a9e7e" : "#d4b896" }}>
                    {doneDelta !== null && doneDelta <= 0 ? `−${Math.abs(doneDelta)}` : `+${doneDelta}`}
                  </p>
                  <p className="mono-label mt-1">heaviness delta</p>
                  <p className="font-mono text-[12px] text-ink-light mt-3 leading-relaxed">
...
function DeltaBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="h-32 w-12 rounded-lg bg-cream-dark border border-border flex items-end overflow-hidden">
        <div

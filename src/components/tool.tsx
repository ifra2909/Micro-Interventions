...
function ReadoutLine({ k, v, vColor = "#17202A" }: { k: string; v: string; vColor?: string }) {
  return (
    <p className="font-mono text-[11.5px] leading-relaxed text-[#17202A]">
      <span className="text-[#4A5868]">{k}</span> <span style={{ color: vColor }}>{v}</span>
    </p>
  );
}
...
function ReadoutPanel({ stage, preview, inference, intervention, analysisStage, delta }: PanelProps) {
  ...
  const orbColor =
    ...
    : live && preview ? (preview.arousal > 0.45 ? "#D4B4A8" : preview.arousal > 0.2 ? "#D4C4A8" : "#A8C4BC")
    : "#A8C4BC";
    ...
    <div className="panel p-7 flex flex-col items-center gap-5">
      ...
      <div className="inset-screen w-full p-5 min-h-[180px]">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`w-1.5 h-1.5 rounded-full ${stage === "analyzing" ? "bg-[#2C3E50]" : "bg-[#A8C4BC]"} ${stage !== "done" ? "animate-pulse" : ""}`} />
        </div>

        {stage === "input" && !live && (
          <div className="font-mono text-[11.5px] leading-[2] text-[#4A5868]">
            <p className="text-[#4A5868]">▸ system idle — awaiting input</p>
            <p className="text-[#4A5868]">three signals feed the match:</p>
            <p><span className="text-[#A8C4BC]">01</span> the situation you describe</p>
            <p><span className="text-[#A8B8C8]">02</span> the emotion language you use</p>
            <p><span className="text-[#D4B4A8]">03</span> what you've already tried</p>
            <p className="text-[#4A5868]">sequence: safety → clarity → growth</p>
            <p className="text-[#A8C4BC]">type anything — messy is welcome<span className="caret">▊</span></p>
          </div>
        )}
        ...
        {stage === "analyzing" && (
          <div className="font-mono text-[11.5px] leading-[2] text-[#4A5868]">
            {ANALYSIS_LINES.slice(0, analysisStage + 1).map((l, i) => (
              <p key={l} className={i === analysisStage ? "text-[#17202A]" : "text-[#4A5868]"}>
                <span className="text-[#A8C4BC]">▸</span> {l}
        ...
        {stage === "done" && inference && (
          <div className="pop-in font-mono text-[11.5px] leading-[2]">
            <p className="text-[#17202A]">✓ session logged</p>
            <ReadoutLine k="mechanism:" v={MECHANISMS[inference.mechanism].label} />
            {delta !== null && (
              <ReadoutLine k="heaviness:" v={`${delta <= 0 ? `${delta}` : `+${delta}`} — ${delta <= 0 ? "lighter. that's the whole game." : "rough round. it still counts."}`} />
            )}
            <p className="text-[#4A5868]">the tool did its job: you felt enough to act.</p>
          </div>
        )}
      </div>
      <p className="text-center text-[11px] font-mono text-[#4A5868] -mt-1">
  ...
  return (
    <div className={`panel p-6 transition-opacity ${confirmed ? "opacity-70" : ""}`}>
      <div className="flex items-baseline justify-between gap-4 mb-3">
        ...
        <p className="font-display text-3xl text-[#17202A] tabular-nums">
          {value}<span className="text-[#4A5868] text-base font-body font-normal"> /10</span>
        </p>
      </div>
    ...
    <section id="tool" className="max-w-5xl mx-auto px-5 sm:px-8 pt-16 sm:pt-24 pb-20">
      <div className="grid lg:grid-cols-12 gap-10 lg:gap-12">
        ...
        <div className="lg:col-span-7">
          {stage === "input" && (
            <div className="pop-in">
              <h1 className="font-display text-[#17202A] mt-5 leading-[1.05] text-[clamp(2.5rem,6vw,4.2rem)]">
                what's weighing<br />on you <em className="italic text-[#4A5868]">right now?</em>
              </h1>
              <p className="text-[#4A5868] text-[15px] sm:text-base leading-relaxed mt-5 max-w-xl">
              ...
              <div className="panel p-2 mt-10 focus-within:border-[#4A5868] transition-colors">
                <textarea
                  ref={taRef}
                  value={text}
                  ...
                  className="w-full bg-transparent px-4 py-3 text-[#17202A] text-[15px] leading-relaxed resize-none outline-none placeholder:text-[#4A5868] min-h-[110px]"
                />
                <div className="flex items-center justify-between px-3 pb-2">
                  <span className="font-mono text-[11px] text-[#4A5868]">{text.length}/600 · ⌘↵ to decode</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {QUICK_CHIPS.map((c) => (
                    <button
                      ...
                      className="btn-ghost px-4 py-2 text-sm hover:text-[#17202A] transition-colors"
                    >
                      {c.label}
                    </button>
                  ))}
              ...
              <p className="font-mono text-[11px] text-[#4A5868] mt-8 leading-relaxed">
                if this is a crisis and not a bad day —{" "}
                <a href="tel:9152987821" className="underline hover:text-[#17202A] transition-colors">iCall: 9152987821</a> or{" "}
                <a href="tel:18602662345" className="underline hover:text-[#17202A] transition-colors">Vandrevala: 1860-2662-345</a>.
            ...
            <div className="pop-in" key={intervention.id}>
              <div className="flex items-center justify-between gap-4">
                <p className="mono-label truncate">
                  you said: <span className="text-[#4A5868] normal-case tracking-normal">"{manual ? "picked from library" : text.trim().slice(0, 72)}{!manual && text.trim().length > 72 ? "…" : ""}"</span>
                </p>
              ...
              <div className="mt-7">
                <h2 className="font-display text-[#17202A] leading-tight text-3xl sm:text-4xl tracking-tight">{inference.headline}</h2>
                <div className="flex items-center gap-3 mt-3">
                  <MechTag id={inference.mechanism} />
                  <span className="font-mono text-[11px] text-[#4A5868]">{MECHANISMS[inference.mechanism].need}</span>
                </div>
              </div>

              {inference.gateNote && (
                <p className="font-mono text-[11px] text-[#17202A] mt-3 bg-[#D4C4A8]/30 px-3 py-1.5 rounded-md inline-block">▸ {inference.gateNote}</p>
              )}
              <p className="text-[#4A5868] text-[15px] leading-relaxed mt-4 max-w-xl">{inference.message}</p>
              ...
              <div className="panel mt-6 overflow-hidden">
                <div className="px-7 pt-7 pb-6 border-b border-[#C2CBD4] flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <h3 className="font-display text-[#17202A] text-2xl tracking-tight">{intervention.title}</h3>
                    <p className="font-mono text-[11px] text-[#4A5868] mt-1.5 leading-relaxed max-w-md">
                      evidence: {intervention.evidence}
                    </p>
                  </div>
                  <span className="font-mono text-[11px] text-[#4A5868] border border-[#C2CBD4] rounded-full px-3 py-1.5 shrink-0">
                ...
                <div className="px-7 py-6">
                  ...
                  <p className="text-[#4A5868] text-sm leading-relaxed max-w-xl">{intervention.why}</p>
                </div>
                        ...
                        <li key={i}>
                          <button
                            ...
                            className={`w-full text-left flex items-start gap-3 px-4 py-3 rounded-xl border transition-all duration-200 ${
                              done ? "border-[#C2CBD4] bg-[#E4E8EC]" : "border-[#C2CBD4] bg-[#D5DCE2] hover:-translate-y-0.5"
                            }`}
                          >
                            <span
                              className={`w-6 h-6 rounded-full border flex items-center justify-center shrink-0 mt-0.5 transition-colors ${
                                done ? "bg-[#A8C4BC] border-[#A8C4BC] text-[#E4E8EC]" : "border-[#C2CBD4] text-[#4A5868]"
                              }`}
                            >
                              {done ? (
                            ...
                            <span className={`text-sm leading-relaxed transition-colors ${done ? "text-[#4A5868] line-through decoration-[#A8C4BC]/30" : "text-[#17202A]"}`}>
                              {s}
                            </span>
                          </button>
                        </li>
                ...
                <div className="px-7 pb-7">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex-1 h-1.5 rounded-full bg-[#C2CBD4] overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-500"
                        style={{ width: `${progress * 100}%`, background: MECHANISMS[intervention.mechanism].color }}
                      />
                    </div>
                    <span className="font-mono text-[11px] text-[#4A5868]">{Math.round(progress * 100)}%</span>
                  </div>
                  ...
                  {!completeReady && (
                    <p className="font-mono text-[11px] text-[#4A5868]">
              ...
              <div className="flex items-center justify-between gap-4 mt-5 flex-wrap">
                <p className="font-mono text-[11px] text-[#4A5868] italic max-w-sm">"{intervention.closer}"</p>
          ...
          {stage === "done" && inference && intervention && (
            <div className="pop-in">
              ...
              <h2 className="font-display text-[#17202A] mt-3 leading-[1.05] text-[clamp(2.2rem,5vw,3.5rem)]">
                unclenched<span className="text-[#A8C4BC]">.</span>
              </h2>
              <p className="text-[#4A5868] mt-4 max-w-xl text-[15px] leading-relaxed">
                You didn't solve the situation — you restored the mechanism underneath it. That's the whole
                trick: feel better <em className="text-[#17202A] not-italic font-medium">first</em>, then handle it. Because you can.
              </p>

              <div className="panel mt-10 p-7 flex flex-col sm:flex-row items-center gap-8">
                <div className="flex items-end gap-6">
                  <DeltaBar label="before" value={moodBefore ?? moodBeforeVal} color="#D4C4A8" />
                  <DeltaBar label="after" value={moodAfterVal} color="#A8C4BC" />
                </div>
                <div className="flex-1 text-center sm:text-left">
                  <p className="font-display text-5xl tabular-nums text-[#17202A]">
                    {doneDelta !== null && doneDelta <= 0 ? `−${Math.abs(doneDelta)}` : `+${doneDelta}`}
                  </p>
                  <p className="mono-label mt-1">heaviness delta</p>
                  <p className="font-mono text-[12px] text-[#4A5868] mt-3 leading-relaxed">
...
function DeltaBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="h-32 w-12 rounded-lg bg-[#D5DCE2] border border-[#C2CBD4] flex items-end overflow-hidden">
        <div

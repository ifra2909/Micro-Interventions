    ...
    <section className="max-w-5xl mx-auto px-5 sm:px-8 py-24">
      <Reveal>
        <SectionHead
          ...
          title={<>what this is.<br /><em className="italic text-ink-light">what this isn't.</em></>}
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <Reveal>
          <div className="panel p-7 h-full">
            ...
            <ul className="space-y-4">
              {IS_LIST.map((t) => (
                <li key={t} className="flex items-start gap-3 text-sm text-ink-light leading-relaxed">
        ...
        <Reveal delay={120}>
          <div className="panel p-7 h-full">
            <p className="mono-label text-rose mb-5">what this isn't</p>
            <ul className="space-y-4">
              {ISNT_LIST.map((t) => (
                <li key={t} className="flex items-start gap-3 text-sm text-ink-light leading-relaxed">
                  <span className="mt-0.5 w-5 h-5 rounded-full bg-rose/8 border border-rose/25 text-rose flex items-center justify-center shrink-0">
      ...
      <Reveal delay={180}>
        <p className="font-display text-xl sm:text-2xl text-ink-light italic text-center mt-12 max-w-2xl mx-auto leading-snug">
  ...
  return (
    <section id="log" className="max-w-5xl mx-auto px-5 sm:px-8 py-24">
      <Reveal>
        <SectionHead
          tag="your log"
          title={<>patterns beat<br /><em className="italic text-ink-light">promises.</em></>}
      ...
      {entries.length === 0 ? (
        <Reveal>
          <div className="rounded-2xl border border-dashed border-border p-12 text-center">
            <p className="font-mono text-[12px] text-ink-muted">▸ no sessions logged yet</p>
            <p className="text-ink-light text-sm mt-2 max-w-sm mx-auto leading-relaxed">
          ...
          <Reveal className="lg:col-span-4">
            <div className="panel p-7 h-full">
              ...
              <div className="flex gap-8 mb-7">
                <div>
                  <p className="font-display text-4xl text-ink">{entries.length}</p>
                  <p className="mono-label mt-0.5">sessions</p>
                </div>
                <div>
                  <p className="font-display text-4xl text-ink">{days}</p>
                  <p className="mono-label mt-0.5">days</p>
                </div>
                <div>
                  <p className="font-display text-4xl" style={{ color: avgDelta <= 0 ? "#7a9e7e" : "#e8a87c" }}>
                ...
                {mechCounts
                  ...
                  .map(({ m, n }, i) => (
                    <div key={m} className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-[10px] text-ink-muted w-4 text-right">{n}</span>
                    </div>
                  ))}
              </div>
              <button
                ...
                className={`mt-7 font-mono text-[11px] px-4 py-2 rounded-full border transition-colors ${
                  confirming ? "border-rose/50 text-rose" : "border-border text-ink-muted hover:text-ink-light"
                }`}
              >
                ...
                {entries.slice(0, 8).map((e) => {
                  ...
                  return (
                    <li key={e.id} className="py-4 flex items-center gap-4 flex-wrap">
                      <span className="font-mono text-[10.5px] text-ink-muted w-24 shrink-0">
                      ...
                      <span className="text-sm text-ink flex-1 min-w-[140px] truncate">
                        {iv.title}
                        {e.manual && <span className="font-mono text-[10px] text-ink-muted ml-2">(library)</span>}
                      </span>
                      {!e.manual && <span className="text-[12px] text-ink-muted italic truncate max-w-[180px] hidden md:block">"{e.snippet}"</span>}
                      <span className="font-mono text-[11px] tabular-nums shrink-0">
                        <span className="text-peach">{e.before}</span>
                      ...
                      <span
                        ...
                        style={
                          d <= 0
                            ? { color: "#7a9e7e", borderColor: "#7a9e7e44", background: "#7a9e7e10" }
                            : { color: "#e8a87c", borderColor: "#e8a87c44", background: "#e8a87c10" }
                        }
                      >
                        {d <= 0 ? `−${Math.abs(d)}` : `+${d}`}
                      </span>
